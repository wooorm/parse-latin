...
function ParseLatin() {
    ...
    self.use('tokenizeWord', self.tokenizeWordPlugins);
    self.use('tokenizeSentence', self.tokenizeSentencePlugins);
    self.use('tokenizeParagraph', self.tokenizeParagraphPlugins);
    self.use('tokenizeRoot', self.tokenizeRootPlugins);
}
...
parseLatinPrototype.use = function (key, plugin) {
    ...
    /**
     * If ware already exists on (not inherited) the
     * object, use it, otherwise create a new list.
     */

    ware = self.hasOwnProperty(wareKey) ? self[wareKey] : [];

    /**
     * Add the plugin(s) to the ware.
     */

    self[wareKey] = ware.concat(
